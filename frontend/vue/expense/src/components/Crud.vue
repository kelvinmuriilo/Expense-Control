...
export default {
  ...
  methods: {
    ...
    loadAsyncData() {
      ...
      let pagina = this.opcoesDefault.page;
      ...
      if (this.pesquisa.length > 0) {
        ...
        console.log(
          document.querySelector(
            "#app > div:nth-child(2) > section > div > div.table-wrapper > table > thead > tr > th:nth-child(1) > div > span:nth-child(1) > div > div > div > input"
          ).value
        );
        urlListar +=
          "/GetById/" +
      ...
      if (this.opcoesDefault.sortField.length > 0) {
        params.push(
          `query[ordem][${this.opcoesDefault.sortField}]=${this.opcoesDefault.sortOrder}`
        );
      }
      ...
      this.$network
        ...
        .then(response => {
          if (response.status == 200) {
            ...
            this.opcoesDefault.perPage = dados.tamanho;
            this.opcoesDefault.total = dados.tamanho * 2;

            var listar = dados.lista == undefined ? dados : dados.lista;
